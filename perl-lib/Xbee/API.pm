...
sub read_api {
    ...
    if ( $buf eq "\x7e" ) {
        ...
        my @chunks = split( //, $buf );
        $api_frame{raw_data} = unpack( "H*", $buf );
        ...
        #Node Discovery
            ...
            $api_frame{net_adress}     = unpack( "H*", _shift( \@chunks, 2 ) );

            # 01 Acked; 02 Broadcast
            $api_frame{receive_options} = unpack( "C",  _shift( \@chunks ) );
            $api_frame{source_adress}   = unpack( "H*", _shift( \@chunks, 2 ) );
            $api_frame{serial}          = unpack( "H*", _shift( \@chunks, 8 ) );
            $api_frame{intentifier} = "";
            while ( my $nic = shift @chunks ) {
                if ( $nic eq "\x00" ) { last; }
                $api_frame{intentifier} .= $nic;
            }
            $api_frame{parent_adress}  = unpack( "H*", _shift( \@chunks, 2 ) );
            ...
            $api_frame{manufactuer_id} = unpack( "n",  _shift( \@chunks, 2 ) );
        }
		...
		elsif( $api_frame{type} == 0x80) {
            $api_frame{serial}  = unpack( "H*", _shift( \@chunks, 8 ) );
            $api_frame{rssi} = unpack( "C", shift @chunks );
            $api_frame{options} = unpack( "C", shift @chunks );
            $api_frame{data} = join( '', @chunks );
		}
		# ADIO Input (Pg 13-14)
		elsif( $api_frame{type} == 0x82 ) {
			$api_frame{serial} = unpack( "H*", _shift( \@chunks, 8 ) );
			$api_frame{rssi} = unpack( "C", shift @chunks );
			$api_frame{options} = unpack( "C", shift @chunks );
			$api_frame{data} = join( '', @chunks );	
...
sub transmit_request {
	my( $self, $dest64, $data ) = @_;
	$self->api_command( "\x00\x01".$dest64."\x00".$data );
