...
{
	my( $http_request ) = @_;
	my $mycnx = getConnection();
	my $bytesSent = $mycnx->send( $http_request->as_string );
	# retry logic
	if( $bytesSent == 0 )
	{
		$cnx = undef; # force reset
		$mycnx = getConnection();
		$bytesSent = $mycnx->send( $http_request->as_string );
		logger( "INFO", "Sent: " . $http_request->as_string . " (Actual: $bytesSent bytes after retry)");
		return;
	}
	logger( "INFO", "Sent: " . $http_request->as_string . " (Actual: $bytesSent bytes)");
}
...
sub main
{
	my $ipAddr = getMyIpAddress();
	logger( "INFO", "Running service on host $ipAddr" );
	# Create
	...
	while (1) {
		...
		if( exists $frame->{type} ) {
			...
			# enrich local address
			$frame->{local_address} = $ipAddr;

			# Hold on!  Pool the data before it goes out if it's a frame from RS232 adapter.
			if( $frame->{serial} eq "0013a200402ceee6" )
			{
